...
/**
 * generic eALS for time complexity baseline
 * 
 * @author Xiaoyu Du
 */
public class MF_eALS_bigW extends TopKRecommender {
	/** Model priors to set. */
	...
	// Since the matrix is over storage, we use a weight function to compute the
	// weights in O(Z) alternatively.
	// DenseMatrix W;
	...
	public MF_eALS_bigW(SparseMatrix trainMatrix, ArrayList<Rating> testRatings, int topK, int threadNum, int factors,
		...
		Wr = new Set[userCount];
		for (int u = 0; u < userCount; u++) {
			Wr[u] = new HashSet<Integer>();
	...
	// use a weight function to compute the weights in O(Z) alternatively
	public double weight(int u, int i) {
		if (Wr[u].contains(i))
			return 1;
		return Wu.row(u, false).inner(Wi.row(i, false));
	}
